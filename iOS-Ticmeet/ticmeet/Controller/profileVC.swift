...
class profileVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func viewWillAppear(_ animated: Bool) {
        if user.userEmail == Auth.auth().currentUser?.email!{
        ...
        else if user.userEmail != Auth.auth().currentUser?.email!{
            ...
            getData(userEmail: user.userEmail!)
            if user.userFollowers?.contains((Auth.auth().currentUser?.email)!) == true{
    ...
    @IBAction func followOrProfileFunc(_ sender: Any) {
        if followOrSettingsButton.titleLabel?.text == "Düzenle"{
            ...
            user.userFollowers?.append((Auth.auth().currentUser?.email)!)
            followUser(addUser: user)
            followOrSettingsButton.setTitle("Takiptesin", for: .normal)
            getData(userEmail: user.userEmail!)
            ...
            user.userFollowers = user.userFollowers!.filter { $0 != Auth.auth().currentUser?.email }
            unfollowUser(selectedUser: user)
            followOrSettingsButton.setTitle("Takip et", for: .normal)
            getData(userEmail: user.userEmail!)
